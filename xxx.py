...
def __get_website_to_db(session, id, file_name):
    try:
        # db_with_retry(collection_listing_crawl.find_one, {"listing_listingId":  { "$gt": 98000001}})
        if db_retry_find_one(collection_listing_crawl, {"listing_listingId": id}):
            return True
        ...
        db_retry_insert_one(collection_listing_crawl, flat_json_data)
        return True
    except Exception as e:
        print(e)
...
def get_website(start):
    ...
    for index in range(100):
        ...
        try_count = 3
        while try_count > 0:
            result = __get_website(session, id, file_name)
            try_count -= 1
            if result:
                break
            time.sleep(5)
